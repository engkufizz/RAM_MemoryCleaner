...
class MonitorWidget(QWidget):
    def __init__(self):
        ...
        # Value labels (RichText with dir='ltr' to force proper % rendering)
        self.ram_val = QLabel(self.panel)
        self.cpu_val = QLabel(self.panel)
        for lbl in (self.ram_val, self.cpu_val):
            lbl.setFont(font)
            lbl.setTextFormat(Qt.RichText)     # allow dir='ltr'
    ...
    # ----- Stats/formatting -----
    @staticmethod
    def ltr_html(text: str) -> str:
        # Wrap in LTR without using embedding control chars
        return f"<span dir='ltr'>{html.escape(text)}</span>"
    ...
    def update_stats(self):
        ...
        # Plain ASCII digits and percent, forced LTR via HTML
        self.ram_val.setText(self.ltr_html(f"{ram_pct}%"))
        self.cpu_val.setText(self.ltr_html(f"{cpu_pct}%"))
    ...
    def set_cleaning_ui(self, cleaning: bool):
        ...
        if cleaning:
            self.ram_val.setText(self.ltr_html("…"))
            self.cpu_val.setText(self.ltr_html("…"))
